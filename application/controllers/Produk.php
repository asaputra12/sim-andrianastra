...
class Produk extends SB_Controller 
{
	...
	function index() 
	{
		...
		$sort = (!is_null($this->input->get('sort', true)) ? $this->input->get('sort', true) : 'id_produk'); 
	...
	function show( $id = null) 
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('tb_produk'); 
		}
	...
	function save() {
		...
		if( !empty($rules) && $this->form_validation->run()){
			...
			$ID = $this->model->insertRow($data , $this->input->get_post( 'id_produk' , true ));
			// Input logs
			if( $this->input->get( 'id_produk' , true ) =='')
			{
